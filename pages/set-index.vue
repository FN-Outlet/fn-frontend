...
export default defineComponent({
  ...
  methods: {
    ...
    async getNews(page, pageSize) {
      const data = await $fetch(`/api/newss?sort=newsdate:desc&filters[Highlight][$eq]=false&filters[newstype][$eq]=set&populate=*&pagination[page]=${page}&pagination[pageSize]=${pageSize}`);
      this.data = data.data;
      this.newsMeta = data.meta.pagination;
      this.loading = false;
    },
    async getNewsHightlight() {
      const { data } = await $fetch(`/api/newss?sort=newsdate:desc&filters[Highlight][$eq]=true&filters[newstype][$eq]=set&populate=*`);
